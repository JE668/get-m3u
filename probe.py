...
if __name__ == "__main__":
  ...
  if os.path.exists(SOURCE_NONCHECK_FILE):
    ...
    if lines:
      # 1. 归集要测试的 IP 和 URL（修复：提取纯 IP 做 key）
      ip_map, url_map = {}, {}
      for line in lines:
        try:
          url = line.split(",", 1)[1]
          # 修复: url.split("/")[2] 返回的是 "ip:port"，需提取纯 IP
          host_port = url.split("/")[2]
          ip_key = host_port.split(":")[0]  # 纯 IP 作为分组 key
          if ip_key not in ip_map: ip_map[ip_key] = []; url_map[ip_key] = []
          ip_map[ip_key].append(line); url_map[ip_key].append(url)
      ...
      # ==========================================
      # 5. 重新拼装存活 IP 并写入 source-m3u.txt（标准 M3U 格式）
      # ==========================================
      ...
      if valid_ips:
        if os.path.exists(RTP_FILE):
          ...
          live_print(f"✨ 测速结束: 存活 {len(valid_ips)} 个 IP | 生成 {len(m3u_lines)-1} 条纯净链接")
        else:
