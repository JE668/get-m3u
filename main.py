import os, re, requests, time, concurrent.futures, random, threading
...
if __name__ == "__main__":
  ...
  # 端口分级：高优先端口先扫
  HIGH_PORTS = [4022, 8000, 8081, 8088, 8888, 8686, 9999, 10000, 10001, 9876, 9090, 8443, 55555, 54321, 1024]
  high_ports = [p for p in all_ports if int(p) in HIGH_PORTS]
  ext_ports = [p for p in all_ports if int(p) not in HIGH_PORTS]
  ...
  if geo_ips:
    ...
    if os.path.exists(RTP_FILE):
      ...
      # 同时输出兼容的自定义格式（供下游项目解析）
      compat_lines = [f"{r.split(',')[0]},http://{ip}/rtp/{r.split('://')[1]}" 
                      for ip in geo_ips for r in rtps]
